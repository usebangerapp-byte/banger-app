...
export async function POST(req: Request) {
  try {
    ...
    // prepare artist + title for link search
    const artist = String(mapped.track_subtitle || "").trim();
    const title = String(mapped.track_title || "").trim();
    ...
    if (
      privateTrackRow?.id &&
      mapped.result_type === "recognized_world" &&
      title &&
      artist &&
      (!privateTrackRow.spotify_url || !privateTrackRow.beatport_url)
    ) {
      try {
        ...
        await supabase
          ...
          .eq("id", privateTrackRow.id);
      } catch (e) {
        console.error("auto link update failed", e);
      }
    }
